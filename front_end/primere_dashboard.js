    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name == 'PrimerePromptSwitch') {
            ...
            nodeType.prototype.onConnectionsChange = function(type, index, connected, link_info) {
                            ...
                            if (link_info.type == '*') {
                                ...
                                for (let i in this.inputs) {
                                    ...
                                    if (this.inputs[i].name.includes(input_name_pos) === true) {
                                        ...
                                        let input_i_pre = this.inputs[(n + 2)];
                                        ...
                                        input_i_pre.type = link_info.type;
                ...
                if (!connected && (this.inputs.length > 3 + converted_count)) {
                ...
                for (let i = 0; i < this.inputs.length; i++) {
                    if (this.inputs[i].name.includes(input_name_pos) === true) {
                        ...
                        let input_i_pre = this.inputs[(i + 2)];
                        ...
                        input_i_pre.name = `${input_name_pre}${slot_i}`;
				...
				let last_pos_slot = this.inputs[this.inputs.length - 3];
                let last_neg_slot = this.inputs[this.inputs.length - 2];
                if (last_pos_slot.name.includes(input_name_pos) === true && last_neg_slot.link != undefined && last_pos_slot.link != undefined) {
                        ...
                        this.addInput(`${input_name_pre}${slot_i}`, this.outputs[2].type);
                ...
                if (this.widgets) {
                    ...
                    var selectMax = Math.round((this.inputs.length / 3) + additionalMax);
