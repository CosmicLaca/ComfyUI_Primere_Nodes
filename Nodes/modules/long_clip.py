...
from ...components.long_clip_model import longclip
import os
import torch
...
class SDLongClipModel(torch.nn.Module, ClipTokenWeightEncoder):
    ...
    def __init__(self, version="openai/clip-vit-large-patch14", device="cpu", max_length=77, freeze=True, layer="last", layer_idx=None, dtype=None, special_tokens={"start": 49406, "end": 49407, "pad": 49407}, layer_norm_hidden_state=True, enable_attention_masks=False, return_projected_pooled=True, **kwargs):  # clip-vit-base-patch32
        super().__init__()
...
class SDXLLongClip:
    def sdxl_longclip(self, clip_name, clip):
...
class SDLongClip:
    def sd_longclip(self, clip_name):
        class EmptyClass:
            pass
...
class FluxLongClip:
    def flux_longclip(self, clip_name, clip):
        clip_clone = clip.clone()
        clip_path = folder_paths.get_full_path("clip", clip_name)
        load_device = model_management.text_encoder_device()
        device = model_management.text_encoder_offload_device()
